...
networkChoices = ['morocco-1', '90u-3'] 
both.add_argument(
    '-n',
    '--network',
    type = str,
    ...
    default='morocco-1',
    help='R|Network to join \nDefault: "90u-3" '+str(networkChoices)+'\n ',
    dest="network")
...
3) Don't install cosmovisor and don't set up a background service
    ...
    if useCosmovisor == "1":
        ...
        subprocess.run(["mkdir -p "+terp_home+"/cosmovisor/upgrades/v4.2.0/bin"], shell=True, env=my_env)
        ...
        subprocess.run(["cp build/terpd "+terp_home+"/cosmovisor/upgrades/v4.2.0/bin"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
...
#     subprocess.run(["mkdir -p "+terp_home+"/cosmovisor/upgrades/v4.2.0/bin"], shell=True, env=my_env)
#     os.chdir(os.path.expanduser(HOME+"/terp-core"))
#     print(bcolors.OKGREEN + "Preparing v4.2.0 Upgrade..." + bcolors.ENDC)
...
def stateSyncInit ():
    ...
    LATEST_HEIGHT= subprocess.run(["curl -s http://test-rpc.terp.network.com:26657/block | jq -r .result.block.header.height"], capture_output=True, shell=True, text=True, env=my_env)
    TRUST_HEIGHT= str(int(LATEST_HEIGHT.stdout.strip()) - 2000)
    TRUST_HASH= subprocess.run(["curl -s \"http://testnet-rpc.terp.network.com:26657/block?height="+str(TRUST_HEIGHT)+"\" | jq -r .result.block_id.hash"], capture_output=True, shell=True, text=True, env=my_env)
    RPCs = "testnet-rpc.terp.network:26657"
    ...
    print(bcolors.OKGREEN + """
# Terp-Core is about to statesync. This process can take anywhere from 5-30 minutes.
...
# 2) No
    ...
    if stateSyncAns == "1":
        ...
        print(bcolors.OKGREEN + "Statesync finished." + bcolors.ENDC)
#         os.chdir(os.path.expanduser(HOME))
...
def testnetStateSyncInit ():
    print(bcolors.OKGREEN + "Replacing trust height, trust hash, and RPCs in config.toml" + bcolors.ENDC)
    LATEST_HEIGHT= subprocess.run(["curl -s http://testnet-rpc.terp.network:26657/block | jq -r .result.block.header.height"], capture_output=True, shell=True, text=True, env=my_env)
    TRUST_HEIGHT= str(int(LATEST_HEIGHT.stdout.strip()) - 2000)
    TRUST_HASH= subprocess.run(["curl -s \"http://testnet-rpc.terp.network:26657/block?height="+str(TRUST_HEIGHT)+"\" | jq -r .result.block_id.hash"], capture_output=True, shell=True, text=True, env=my_env)
    RPCs = "testnet-rpc.terp.network:26657"
    subprocess.run(["sed -i -E 's/enable = false/enable = true/g' "+terp_home+"/config/config.toml"], shell=True)
    subprocess.run(["sed -i -E 's/rpc_servers = \"\"/rpc_servers = \""+RPCs+"\"/g' "+terp_home+"/config/config.toml"], shell=True)
    subprocess.run(["sed -i -E 's/trust_height = 0/trust_height = "+TRUST_HEIGHT+"/g' "+terp_home+"/config/config.toml"], shell=True)
...
# install's snapshot from snapshot provider
...
def setupTestnet ():
    ...
    subprocess.run(["terpd init " + nodeName + " --chain-id=90u-3 -o --home "+terp_home], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
    print(bcolors.OKGREEN + "Downloading and Replacing Genesis..." + bcolors.ENDC)
    subprocess.run(["curl https://raw.githubusercontent.com/terpnetwork/test-net/master/90u-3/stock-genesis.json >> "+terp_home+"/config/genesis.json"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
...
def clientSettings():
    if networkType == NetworkType.MAINNET:
        ...
        subprocess.run(["rm "+terp_home+"/config/client.toml"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run(["terpd init " + nodeName + " --chain-id=morocco-1 -o --home "+terp_home],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, env=my_env)
        ...
        #subprocess.run(["sed -i -E 's|node = \"tcp://localhost:26657\"|node = \"https://rpc.terp.network:443\"|g' "+terp_home+"/config/client.toml"], shell=True)
        ...
        subprocess.run(["sed -i -E 's/chain-id = \"\"/chain-id = \"90u-3\"/g' "+terp_home+"/config/client.toml"], shell=True)
        subprocess.run(["sed -i -E 's|node = \"tcp://localhost:26657\"|node = \"https://testnet-rpc.terp.network\"|g' "+terp_home+"/config/client.toml"], shell=True)
        subprocess.run(["clear"], shell=True)
        clientComplete()
...
def initNodeName ():
    ...
    if nodeName and networkType == NetworkType.MAINNET and node == NodeType.FULL:
        ...
        subprocess.run(["rm -r "+terp_home], stdout=subprocess.DEVNULL,
                       stderr=subprocess.DEVNULL, shell=True, env=my_env)
        subprocess.run(["rm -r "+HOME+"/.terpd"], stdout=subprocess.DEVNULL,
                       stderr=subprocess.DEVNULL, shell=True, env=my_env)
        setupMainnet()
